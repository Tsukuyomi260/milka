...
export default function ArtisanDetail() {
  ...
  if (!artisan) {
    return (
      <Layout>
        <p>{t('artisan.notFound')}</p>
        <Link to="/">{t('artisan.backHome')}</Link>
      </Layout>
    );
  }
  ...
  return (
    <Layout>
      <Link
        to="/"
        style={{ display: 'inline-flex', alignItems: 'center', gap: 8, marginBottom: 20, color: 'var(--text-secondary)', fontSize: '0.9rem' }}
      >
        <ArrowLeft size={20} /> {t('artisan.back')}
      </Link>
      <div className="glass-card glass-card-strong" style={{ padding: 24, textAlign: 'center' }}>
        ...
        <p style={{ margin: '12px 0 0 0', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6 }}>
          <Star size={18} fill="var(--milka-red)" color="var(--milka-red)" /> {artisan.note} · {t('artisan.reviewsCount', { count: artisan.avis })}
        </p>
      </div>
      ...
      <div style={{ marginTop: 24, display: 'flex', gap: 12 }}>
        <button type="button" className="btn-primary" style={{ flex: 1 }}>{t('artisan.contact')}</button>
        <button type="button" className="btn-ghost">{t('artisan.appointment')}</button>
      </div>
    </Layout>
  );
}
